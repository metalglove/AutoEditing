...
public class EntryPoint
{
    ...
    public void FromVegas(Vegas vegas)
    {
        try
        {
            // v2 runs against Untitled.relinked.veg (media already relinked + saved by
            // make-relinked-copy.cs in a prior session). No in-script relinking here --
            // that was the source of the ReplaceWith/RefreshNeeded COM-error cascade seen
            // in v1's diagnostics. This run also verifies (Codex Priority 7) that the
            // relink survived save+reopen: log offline status without attempting any fix.
            VerifyMediaOnlineStatus(vegas.Project);

            string projectJson = ExportProject(vegas);

            string outPath = @"C:\VEGAS\project-inspection-v3.json";
            File.WriteAllText(outPath, projectJson, new UTF8Encoding(false));

            string diagPath = @"C:\VEGAS\project-inspection-v3-diagnostics-raw.txt";
        ...
        catch (Exception ex)
        {
            ...
            {
                File.WriteAllText(
                    @"C:\VEGAS\project-inspection-v3-FATAL.txt",
                    ex.ToString());
            }
            catch { }
        }
    ...
    // ---------- Media state verification (read-only; Priority 7) ----------
    ...
    static void VerifyMediaOnlineStatus(Project project)
    {
        ...
        while (en.MoveNext())
        {
            ...
            if (isOff)
            {
                ...
                LogDiag("VerifyMedia: STILL OFFLINE after save+reopen: " + p);
            }
        }
        LogDiag("VerifyMedia: total=" + total + " offline=" + offline + " (should be 0 if relink persisted through SaveProject)");
    }
    ...
    static string ExportProject(Vegas vegas)
    {
        ...
        props.Add(Prop("schemaVersion", JS("3.0")));
        props.Add(Prop("inspectorVersion", JS("3.0")));
    ...
    static string ExportEvent(TrackEvent ev, int trackIndex, int eventIndex)
    {
        ...
        // Int64 EventID values (e.g. 3458764513820540000) exceed JS's safe-integer range;
        // serialize as a string so consumers don't silently lose precision.
    ...
    static string ExportEffect(Effect fx, int index, bool hasEventContext, double eventStartSeconds)
    {
        ...
        // Classic (non-OFX) effect automation: Keyframe only exposes Position/Type/IsValid --
        // Keyframe.Preset has NO getter (write-only), confirmed via reflection, so classic
        // per-parameter numeric automation (e.g. exact Pitch Shift semitone amount) is not
        // reachable through this object model at all; only preset-switch timing is.
        try
        {
    ...
    // A keyframe's raw (time, value) before we compute prev/next/baseline context.
    ...
    // effectChainIndex/pluginUniqueId/effectDescription identify which effect this
    // parameter belongs to; hasEventContext/eventStartSeconds let us additionally
    // compute a timeline-absolute time alongside the raw (event- or plugin-relative)
    // OFXKeyframe.Time -- the exact frame of reference for OFX keyframe time wasn't
    // documented anywhere we found, so both are exported and the correct one should
    // be confirmed empirically (e.g. against known marker/kill timestamps).
        ...
        bool hasEventContext, double eventStartSeconds)
    {
        ...
        try
        {
            ...
            // Shared enrichment for the four numeric-keyframe cases (Double/Integer/Boolean
            // all populate `raw`; Choice already wrote its own keyframes array above).
            if (raw.Count > 0)
            {
                List<string> kfs = new List<string>();
